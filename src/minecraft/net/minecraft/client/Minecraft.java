...
public abstract class Minecraft implements Runnable {
	...
	public void startGame() throws LWJGLException {
		...
		Display.setTitle("Minecraft Minecraft Beta 1.5_01");
		...
		this.field_25001_G = new StatFileWriter(this.session, this.mcDataDir);
	...
	public void displayGuiScreen(GuiScreen var1) {
		if (!(this.currentScreen instanceof GuiUnused)) {
			...
			if (var1 != null) {
				if(var1 instanceof ClickGUI) {
					((ClickGUI) var1).canSelectNewTab = false;
					((ClickGUI) var1).justInitialized = true;
				}
		...
		try {
			...
			while(this.running && (this.mcApplet == null || this.mcApplet.isActive())) {
				...
				if (this.theWorld != null && !this.theWorld.multiplayerWorld) {
					//int a = 0;
					//int svd = this.theWorld.field_1051_z.size();
					//while(this.theWorld.func_6465_g()) {
					//	a += 500;
					//	svd = this.theWorld.field_1051_z.size();
					//}
					//a += svd;
				...
				if (!Display.isActive()) {
					//XXX fix inability to enter fullscreen
					//if (this.fullscreen) {
	...
	public void toggleFullscreen() {
		try {
			...
			if (this.fullscreen) {
				...
				if(mc_frame != null) {
					GraphicsDevice device = mc_frame.getGraphicsConfiguration().getDevice();
					device.setFullScreenWindow(mc_frame);
				}
			} else {
				if (this.mcCanvas != null) {
					this.displayWidth = this.mcCanvas.getWidth();
			...
			this.setIngameNotInFocus();
			if(mc_frame == null) { //TODO: why multimc
										...
										do {
											if (!Keyboard.next()) {
												if (this.currentScreen == null) {
													...
													if (Mouse.isButtonDown(1) && (FastPlaceHack.instance.status || (float)(this.ticksRan - this.field_6302_aa) >= this.timer.ticksPerSecond / 4.0F) && this.inGameHasFocus) {
														this.clickMouse(1);
														this.field_6302_aa = this.ticksRan;
													}
												}
	...
	public boolean usePortal_antiRecurse = false;
	public void usePortal() {
		//XXX MaybeAClient: Fixes rare StackOverFlow
		if(usePortal_antiRecurse) return;
		usePortal_antiRecurse = true;
