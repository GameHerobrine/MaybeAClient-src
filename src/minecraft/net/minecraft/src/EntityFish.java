        ...
        if (this.field_6388_l > 0) {
            ...
            if (this.worldObj.multiplayerWorld && AutoFishHack.instance.status) {
            	if(!hasLastRecordedY) {
            		...
            		this.hasLastRecordedY = true;
            	}else {
            		if(this.posY != this.lastRecordedY) {
            			double diff = this.posY-this.lastRecordedY;
            			if(diff < -0.1) {
            				byte max = 5;
                            double inwater = 0.0D;

                            for(int i = 0; i < max; ++i) {
                                double var14 = this.boundingBox.minY + (this.boundingBox.maxY - this.boundingBox.minY) * (double)(i + 0) / (double)max - 0.125D + 0.125D;
                                double var16 = this.boundingBox.minY + (this.boundingBox.maxY - this.boundingBox.minY) * (double)(i + 1) / (double)max - 0.125D + 0.125D;
                                ...
                                if (this.worldObj.isAABBInMaterial(var18, Material.water)) {
                                    inwater += 1.0D / (double)max;
                                }
                            }
                            
                            if(inwater > 0 && this.fishing) {
                            	PlayerUtils.useItem();
                            	PlayerUtils.useItem();
                            	this.fishing = false;
                            	this.fishingTicksCheck = 0;
                            }
            			}
