...
import net.skidcode.gh.maybeaclient.gui.mapart.GuiMapArtCreator;
...
public class GuiMainMenu extends GuiScreen {
	...
	public GuiMainMenu() {
		...
		try {
			...
			var1.add("Free cliff?");
			var1.add("@freehij gib enchantile");
			if(Client.debug) ;
	...
	public void initGui() {
		...
		this.controlList.add(new GuiButton(5, this.width - 100, this.height-20-2-12, 98, 20, "Mapart creator"));
	...
	protected void actionPerformed(GuiButton var1) {
		...
		if(var1.id == 5) {
			this.mc.displayGuiScreen(new GuiMapArtCreator(this));
		}
		...
		if (var1.id == 2) {
			Client.class.getClass(); //XXX server selector
